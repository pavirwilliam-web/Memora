...
const app = express();
app.use(express.json({ limit: '15mb' })); // limite large car les photos (base64) passent par ici
...
app.post('/api/ask', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const body = {
      ...
      max_tokens: (max_tokens || 1000) + 500, // marge pour le raisonnement interne du modèle
    ...
    if (!apiRes.ok) return res.status(apiRes.status).json({ error: data.error?.message || 'Erreur API Gemini.' });
    const text = data.choices?.[0]?.message?.content || '';
    res.json({ text, raw: data });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'Erreur serveur pendant l\'appel à Claude.' });
  }
});
