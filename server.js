...
app.post('/api/storage', requireAuth, (req, res) => {
  ...
  if (shared) {
    ...
    if (key.startsWith('user-status:') && parsed && !parsed.premium) {
      const before = db.shared[key] || {};
      const elapsed = parsed.trialStart ? Date.now() - parsed.trialStart : 0;
      const wasLocked = before.trialStart ? (Date.now() - before.trialStart) >= (15 * 60 * 1000) : false;
      const nowLocked = !!parsed.trialStart && elapsed >= (15 * 60 * 1000);
      const manualRequest = parsed.premiumRequested && !before.premiumRequested;
      if (((nowLocked && !wasLocked) || manualRequest) && !parsed.dismissed) {
        notifyAdmins({ title: '💎 Nouvelle demande premium', body: `${key.slice('user-status:'.length)} souhaite passer en mode premium.`, tag: key });
      }
    }
