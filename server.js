...
app.post('/api/ask', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Filet de sécurité en plus de la file d'attente ci-dessus : si Google renvoie quand même
    // un 429 (429 "RESOURCE_EXHAUSTED" — par ex. juste après un redéploiement, ou un pic
    // vraiment inhabituel), on retente jusqu'à 2 fois avec une pause croissante avant
    // d'abandonner, ce qui absorbe les derniers cas résiduels sans que l'élève voie d'erreur.
    let apiRes, data;
    const maxAttempts = 3;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      // même une petite marge croissante pour les cas où le quota met un peu plus de temps
      // à se libérer.
