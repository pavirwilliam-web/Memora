...
app.get('/api/rooms', requireAuth, (req, res) => {
  const list = Object.values(db.rooms).map(roomPublicView).sort((a, b) => (b.isDefault - a.isDefault) || (a.createdAt - b.createdAt));
  res.json({ rooms: list });
});
...
app.post('/api/ask', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Le plan gratuit de Gemini limite le nombre de requêtes par minute (429 "RESOURCE_EXHAUSTED")
    // — ça arrivait par exemple en ajoutant plusieurs notions d'affilée. Avant, la 1ère erreur 429
    // remontait directement à l'utilisateur avec un message peu clair ("voir les logs du serveur").
    // Maintenant : jusqu'à 2 nouvelles tentatives avec une pause croissante (1,5s puis 3s) avant
    // d'abandonner, ce qui absorbe la plupart des pics passagers sans que l'élève voie d'erreur.
    let apiRes, data;
    const maxAttempts = 3;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      apiRes = await fetch('https://generativelanguage.googleapis.com/v1beta/openai/chat/completions', {
        method: 'POST',
        headers: {
      ...
      if (apiRes.ok || apiRes.status !== 429 || attempt === maxAttempts) break;
